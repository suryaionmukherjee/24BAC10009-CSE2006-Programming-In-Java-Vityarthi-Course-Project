...
## Features
*   **Player Management:** Add, update, and view player records with specific roles (Batsman, Bowler) and statistics.
*   **Dynamic Roster Filtering:** Filter and display available players based on their assigned roles.
*   **Match Performance Logging:** Record match results and individual player statistics (runs scored, wickets taken).
*   **Multithreaded Simulation:** Utilizes background threads to simulate match countdowns and processing delays.
*   **File I/O Export:** Export complete team rosters and historical match logs to external text files for easy sharing.
...
4. Select **Option 3** to filter the roster by a specific role (e.g., "Bowler") and verify the sorted output.
...
8. Select **Option 7** to safely exit the application and close the database connection.
